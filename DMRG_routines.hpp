...
namespace ajaj {
  ...
  /** A container that stores blocks and provides an interface to retrieve them.*/
  class BlocksStructure {
  private:
    std::string Name_; //name for file storage
    const Basis* BasisPtr_; //shouldn't change. Using a ptr to a const means we can use the default equals provided by the compiler
  ...
  protected:
    ...
    std::string SetName(const std::string& NewName, uMPXInt a){
  ...
  public:
    BlocksStructure(const Basis& Spectrum) : BasisPtr_(&Spectrum), num_vertices_(0), numLeft_(0), numMiddle_(0) {};
    ...
    const EigenStateArray& getSpectrum() const {return *BasisPtr_;}
    const Basis& basis() const {return *BasisPtr_;}
  ...
  public:
    ...
    bool fill(const std::string& initfilename, uMPXInt size); //Create superblock by loading in an MPS and applying Hamiltonian to it.
    bool fill(const c_specifier_array& cspec, uMPXInt size);
  ...
  private:
    ...
    void ShiftProjectorStatePair(bool dir, uMPXInt new_pos);
