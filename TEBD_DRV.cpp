...
int main(int argc, char** argv){
  ...
  if (RuntimeArgs.is_valid()){
    ...
    if (!myModel.times().size()){ //need this for builtin models, with old style coupling params
      ...
      finrun.evolve(number_of_time_steps,measurements,CHI/*bond dimension*/,trunc,measurement_interval,save_all_flag);
      if (finrun.good()){
	return 0;
      }
    }
    //if we do have time dep couplings...
      ...
      finrun.evolve(num_1,measurements,CHI/*bond dimension*/,trunc/*min s val*/,measurement_interval,save_all_flag);
	...
	finrun.evolve(num,measurements,CHI/*bond dimension*/,trunc/*min s val*/,measurement_interval,save_all_flag);
	...
	finrun.evolve(number_of_time_steps,measurements,CHI/*bond dimension*/,trunc/*min s val*/,measurement_interval,save_all_flag);
      }
