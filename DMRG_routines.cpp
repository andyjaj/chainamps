...
namespace ajaj {
  ...
  BlocksStructure::BlocksStructure(const std::string&  Name, const Basis& Spectrum, uMPXInt num_vertices, uMPXInt numLeft, uMPXInt numMiddle) : Name_(Name), BasisPtr_(&Spectrum),LeftBlock(Spectrum),RightBlock(Spectrum),num_vertices_(num_vertices),numLeft_(numLeft),numMiddle_(numMiddle) {
  ...
  bool BlocksStructure::load_left_block(){
    std::stringstream leftname;
    leftname << Name_ << "_Left_" << left_size() << ".BLOCK";
    LeftBlock=std::move(load_MPX_matrix(leftname.str(),*BasisPtr_));
    return LeftBlock.isEmpty();
  }
  bool BlocksStructure::load_right_block(){
    std::stringstream rightname;
    rightname << Name_ << "_Right_" << right_size() << ".BLOCK";
    RightBlock=std::move(load_MPX_matrix(rightname.str(),*BasisPtr_));
  ...
  SuperBlock::SuperBlock(const std::string& Name, const MPO_matrix& H, const State& TargetState, uMPXInt num_vertices, uMPXInt numLeft, uMPXInt numMiddle) : BlocksStructure(Name,H.basis(),num_vertices,numLeft,numMiddle),H_ptr_(&H),TargetState_(TargetState),CentralDecomposition(H.basis()) {
  ...
  bool SuperBlock::fill_(FiniteMPS& F){
    if (F.total_charges()==TargetState_){
      double norm=F.mixed(F.size()/2); //put it in a mixed state
	...
	if (F.size()>=2){
	  initial_2(MakeInitialLeftBlock(LeftH,F.matrix(1,1)),MakeInitialRightBlock(RightH,F.matrix(F.size(),0)));
	}
      }
      
      for (uMPXInt p=2; p<=F.size()/2;++p){
	insert_2(MakeLeftBlock(getLeftBlock(),getH(),F.matrix(p,1)),MakeRightBlock(getRightBlock(),getH(),F.matrix(F.size()+1-p,0)));
      }
      
      CentralDecomposition.RightMatrix=F.matrix(F.size()/2 +1,0);
      CentralDecomposition.LeftMatrix=F.matrix(F.size()/2,1);

      std::vector<std::complex<double> > FLambda(F.lambda().GetDiagonal());
      CentralDecomposition.Values=std::vector<double>(FLambda.size());
      std::transform(FLambda.begin(),FLambda.end(),CentralDecomposition.Values.begin(),[](std::complex<double> c) -> double { return c.real(); });
      if (size()==F.size()){
	return 1;
      }
    }

    std::cout << "Cannot fill superblock, input is inconsistent with initial parameters." <<std::endl;
    return 0;
  }

  
  bool SuperBlock::fill(const std::string& initfilename, uMPXInt size){
    FiniteMPS F(basis(),initfilename,name(),size,0,size/2); //takes original state and copies it (also overwrites original!)
    return fill_(F);
  }
  
  bool SuperBlock::fill(const c_specifier_array& cspec, uMPXInt size){
    //create the state itself
    FiniteMPS F(basis(),name(),size,cspec);
    return fill_(F);
  }
  ...
  Data SuperBlock::grow_two_vertex(uMPXInt chi, double truncation){
    ...
    else { /*make new left and right blocks, insert dummy vertices ready for solving*/
    ...
    //solve
    Data energy;
    CentralDecomposition=TwoVertexSVD(TwoVertexWavefunction(getLeftBlock(),getH(),getRightBlock(),nullptr,size(),energy,&(pred_.Guess),0),chi,truncation);
    CentralDecomposition.SquareRescale(1.0);
    CentralDecomposition.store(getName(),left_size()+1,right_size()+1,1);
    //CentralDecomposition.OutputPhysicalIndexDensities(DensityFileStream_);
  ...
  Data SuperBlock::move_right_two_vertex(uMPXInt chi, double truncation){
    ...
    CentralDecomposition=TwoVertexSVD(TwoVertexWavefunction(getLeftBlock(),getH(),getRightBlock(),nullptr,size(),energy,&(pred_.Guess),0),chi,right_size()<=1 ? -0.0 : truncation);
    CentralDecomposition.SquareRescale(1.0);
  ...
  Data SuperBlock::move_left_two_vertex(uMPXInt chi, double truncation){
    ...
    CentralDecomposition=TwoVertexSVD(TwoVertexWavefunction(getLeftBlock(),getH(),getRightBlock(),nullptr,size(),energy,&(pred_.Guess),0),chi,left_size()<=1 ? -0.0 : truncation);
    CentralDecomposition.SquareRescale(1.0);
    CentralDecomposition.store(getName(),left_size()+1,right_size()+1,left_size()==right_size());
    //CentralDecomposition.OutputPhysicalIndexDensities(DensityFileStream_);
    double S_E(entropy(CentralDecomposition.Values));
