...
namespace ajaj {
  ...
  BlocksStructure::BlocksStructure(const std::string&  Name, const Basis& Spectrum, uMPXInt num_vertices, uMPXInt numLeft, uMPXInt numMiddle) : Name_(Name), SpectrumPtr_(&Spectrum),LeftBlock(Spectrum),RightBlock(Spectrum),num_vertices_(num_vertices),numLeft_(numLeft),numMiddle_(numMiddle) {
  ...
  SuperBlock::SuperBlock(const std::string&  OldName, const std::string&  NewName,  const MPO_matrix& H, uMPXInt num_vertices, const c_specifier_array& cspec) : BlocksStructure(NewName,H.basis(),num_vertices),H_ptr_(&H),TargetState_(H.basis().Identity()),CentralDecomposition(H.basis()){
    ...
    //First load the state and put it in centred, mixed canonical form
    {
      ajaj::FiniteMPS Ftemp(H.basis(), OldName, num_vertices,cspec);
      std::complex<double> val=Ftemp.mixed_canonical(num_vertices/2,NewName); //store a mixed canonical copy
      //TargetState_=Ftemp.get_state_charges //what are the charges of this state? Use them to record TargetState_
    }
