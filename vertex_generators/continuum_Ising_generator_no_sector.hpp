#ifndef THEORY_ISING_NS_H
#define THEORY_ISING_NS_H
...
namespace continuumIsingNoSector {
  ...
  ajaj::Vertex VertexGenerator(const ajaj::VertexParameterArray& inputs)
  {
    ...
    ModelVertex.ChargeRules.push_back(2); //Z_2 quantum number, needed for matrix elements, but can be dropped later
    ...
    //copy over with sorted spectrum
    ModelVertex.Spectrum=std::move(TempSpectrum);

    //overwrite charge rules

    ModelVertex.ChargeRules=ajaj::QNVector(1,0);
